...
from app.rag.milvus import has_collection, health


async def main() -> None:
    settings = get_settings()
    milvus_ready = await health()
    collection_ready = await has_collection() if milvus_ready else False
    ...
    rag_ready = bool(settings.rag_enabled and milvus_ready and collection_ready and embedding_ready and document_count > 0)
    print(
        {
