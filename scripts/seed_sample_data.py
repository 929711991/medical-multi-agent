...
async def seed() -> None:
    ...
    async with get_session_factory()() as session:
        ...
        await session.flush()
        session.add_all([
            MedicalVisit(patient_id="PT-CARDIO", visit_time=now - timedelta(days=1), department="心内科", chief_complaint="活动后胸痛 2 天", record_json={"blood_pressure": "168/102 mmHg", "pulse": "96 bpm", "sandbox": True}),
            LabResult(patient_id="PT-CARDIO", observed_at=now - timedelta(hours=20), test_name="高敏肌钙蛋白 I", value="0.018 ng/mL", reference_range="<0.026 ng/mL", abnormal_flag="normal"),
            ImagingReport(patient_id="PT-CARDIO", observed_at=now - timedelta(hours=18), modality="ECG", body_part="心脏", findings="窦性心律，部分导联 ST-T 非特异性改变", impression="建议结合症状及动态心电图复查"),
            Medication(patient_id="PT-CARDIO", name="氨氯地平", dose="5 mg qd", route="口服", started_at=now - timedelta(days=180)),
            Allergy(patient_id="PT-CARDIO", substance="青霉素", reaction="皮疹", severity="mild", observed_at=now - timedelta(days=1000)),
            MedicalVisit(patient_id="PT-GASTRO", visit_time=now - timedelta(days=2), department="消化内科", chief_complaint="上腹痛伴恶心 3 天，呕吐 1 次", record_json={"temperature": "37.2 C", "abdomen": "上腹轻压痛", "sandbox": True}),
            LabResult(patient_id="PT-GASTRO", observed_at=now - timedelta(days=2), test_name="血常规白细胞", value="9.2 x10^9/L", reference_range="3.5-9.5 x10^9/L", abnormal_flag="normal"),
            MedicalVisit(patient_id="PT-LOW", visit_time=now - timedelta(days=7), department="全科", chief_complaint="轻微鼻塞 1 天，无发热", record_json={"temperature": "36.6 C", "sandbox": True}),
        ])
