...
from app.rag.redis_store import health as redis_health, has_index
...
async def knowledge_status() -> KnowledgeStatusResponse:
    ...
    redis_ready = await redis_health() if settings.rag_enabled else False
    index_ready = await has_index() if redis_ready and settings.rag_enabled else False
    embedding_ready = bool(settings.embedding_model and settings.embedding_api_key)
    rag_ready = bool(settings.rag_enabled and redis_ready and index_ready and embedding_ready and ready_documents > 0)
